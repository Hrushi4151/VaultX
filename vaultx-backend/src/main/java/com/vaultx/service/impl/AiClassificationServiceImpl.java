...
public class AiClassificationServiceImpl implements AiClassificationService {
    ...
    public AiDocumentAnalysisDto analyzeDocument(UUID documentId) {
        ...
        OcrResult ocrResult = ocrResultRepository.findFirstByDocumentIdOrderByProcessedAtDesc(documentId).orElse(null);
        String ocrText = ocrResult != null && ocrResult.getExtractedText() != null 
                ? ocrResult.getExtractedText() 
                : "";

        // 1. We now strictly use the OCR text generated by the Python OCR Microservice.
        // Tika and PDFBox have been removed per user request.

        // 2. Try Google Gemini Vision AI if API Key is configured
        AiDocumentAnalysisDto geminiResult = analyzeWithGemini(doc, ocrText, null);
    ...
    @Override
    @Transactional
    public DocumentDto applySuggestions(UUID documentId, String suggestedName, String categoryName, String collectionName, List<String> tags, String ocrText) {
        Document doc = documentRepository.findById(documentId)
                .orElseThrow(() -> new ResourceNotFoundException("Document", "id", documentId.toString()));
        ...
        if (ocrText != null && !ocrText.trim().isEmpty()) {
            OcrResult ocrResult = ocrResultRepository.findFirstByDocumentIdOrderByProcessedAtDesc(documentId).orElse(
                    OcrResult.builder().document(doc).status("COMPLETED").build()
            );
            ocrResult.setExtractedText(ocrText);
    ...
    private AiDocumentAnalysisDto analyzeWithGemini(Document doc, String textToAnalyze, byte[] optionalFileBytes) {
        ...
        try {
            ...
            if (doc.getMimeType() != null && doc.getMimeType().startsWith("image/") && doc.getFileSize() != null && doc.getFileSize() < 5 * 1024 * 1024) {
                try {
                    byte[] imgBytes = optionalFileBytes;
                    if (imgBytes == null) {
    ...
    @Override
    public AiDocumentAnalysisDto analyzePreview(org.springframework.web.multipart.MultipartFile file) {
        if (geminiApiKey == null || geminiApiKey.trim().isEmpty()) {
            return null;
        }

        try {
            // 1. Extract Text via Python OCR
            String extractedText = extractTextFromPythonOcr(file);

            // 2. Mock a Document object for analyzeWithGemini
            Document mockDoc = new Document();
            mockDoc.setId(UUID.randomUUID());
            mockDoc.setOriginalFilename(file.getOriginalFilename());
            mockDoc.setDisplayName(file.getOriginalFilename());
            mockDoc.setMimeType(file.getContentType());
            mockDoc.setFileSize(file.getSize());

            // 3. Call Gemini API
            return analyzeWithGemini(mockDoc, extractedText, file.getBytes());
        } catch (Exception e) {
            log.error("Failed to generate AI preview for file {}", file.getOriginalFilename(), e);
            return null;
        }
    }

    private String extractTextFromPythonOcr(org.springframework.web.multipart.MultipartFile file) {
        try {
            String filename = file.getOriginalFilename() != null ? file.getOriginalFilename() : "upload.file";
            log.info("Sending file to Python AI Service for OCR Preview: {}", filename);
            ...
            log.error("Error communicating with Python OCR Service during preview", e);
            return "";
        }
    }
}
